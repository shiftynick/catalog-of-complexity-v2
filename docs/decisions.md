...
- **`instance_population` + `lineage`** (class-level): how many instances exist (~90 tropical cyclones/yr; ~7,000 languages; 1 known planetary climate) and whether instances form lineages (languages famously do — phylogenetic trees; storms don't). Epistemically load-bearing: n=1 classes can't support class-level statistics. Most likely to be pulled from the lot first — the tropical-cyclones class entry will want it immediately.
