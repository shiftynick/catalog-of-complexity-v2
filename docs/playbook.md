...
> **Status (2026-07-02, v0.2):** Battle-tested by hand on tropical-cyclones + financial-markets (Phase 3), encoded and run at scale on the remaining 8 pilot classes (Phase 4), then upgraded to **pipeline v2** and proven again on expansion batch 1 (5 classes, 37 agents, zero blocker flags). The workflow at [.claude/workflows/entry-pipeline.js](../.claude/workflows/entry-pipeline.js) is now parameterized — invoke with `args: {batchFile, date, researchDir}` (per-class hint blocks live in [.claude/workflows/batches/](../.claude/workflows/batches/)) — and the fixer stage is folded in, with each entry gated on `analysis/validate.mjs` passing before the workflow returns. The orchestrator still triages the returned flags and does the draft→verified status bump. Three additions the pilot phases proved essential beyond the steps below: a **cross-entry commensurability audit** after all entries land, **targeted primary-reads** to settle any entry-vs-anchor conflicts (reads have corrected the ANCHOR as well as the entry — verify in both directions), and the **machine validator** as a conformance floor beneath the four semantic verifiers.
