...
**IRR re-run (2026-07-16)** — the three at-or-below-chance columns were re-rated by a fresh blind panel against the boundary-clarified anchors, same design and briefs ([results](../research/v0.3/validation/irr-rerun/results.md)): **the clarifications worked in proportion to their dose.** Criticality (4 rules) went from α=−0.27 to blind unanimity; tipping_transitions (3 rules) from −0.03 to **0.745**; emergence (1 rule) stayed at chance (−0.12) and needs a worked-contrast pair at its 2/3 boundary. Two methodological findings: rater bias flipped from lenient (+0.05…+0.35) to uniformly severe (−0.33…−0.40) — the rules all bite downward — and the evidence-thin briefs now floor criticality wherever the packet lacks event statistics, so rater-vs-catalog calibration claims on that column need enriched briefs first.
